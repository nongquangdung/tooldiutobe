...
class VoiceCard(QFrame):
    ...
    def setup_ui(self):
        ...
        gender_label = QLabel(f"👤 {gender.title()}")
        ...
        quality_label = QLabel(f"⭐ {quality}/10")
        ...
        provider_label = QLabel(f"🔧 {provider}")
...
class VoiceGenerationThread(QThread):
    ...
    def run(self):
        ...
        try:
            self.progress_update.emit("🚀 Starting voice generation...")
            
            # Generate voice
            result = self.generator.generate_voice(self.request)
            
            if result.success:
                self.progress_update.emit("✅ Voice generation completed!")
            else:
                self.progress_update.emit("❌ Voice generation failed!")
            ...
            self.progress_update.emit(f"❌ Generation error: {str(e)}")
            self.generation_complete.emit({
                "success": False,
                "error_message": str(e)
            })
...
class ChatterboxVoicesTab(QWidget):
    ...
    def create_generation_panel(self) -> QWidget:
        ...
        # Header
        header_label = QLabel("🚀 Voice Generation")
        header_label.setFont(QFont("Arial", 14, QFont.Bold))
        header_label.setStyleSheet("color: #2C3E50; margin: 10px 0;")
        layout.addWidget(header_label)
        
        # Selected voice info
        self.selected_voice_group = QGroupBox("🎯 Selected Voice")
    ...
    def select_voice(self, voice_id: str):
        ...
        self.progress_label.setText(f"✅ Selected: {voice_id}")
    ...
    def on_generation_complete(self, result: Dict[str, Any]):
        ...
        if result["success"]:
            msg = f"✅ Voice generated successfully!\n\n"
            msg += f"🎙️ Voice: {result['voice_used']}\n"
            msg += f"🔧 Provider: {result['provider_used']}\n"
            msg += f"⏱️ Time: {result['generation_time']:.2f}s\n"
            msg += f"⭐ Quality: {result['quality_score']}/10\n"
            msg += f"📁 Saved to: {result['output_path']}"
            
            QMessageBox.information(self, "Generation Complete", msg)
            self.progress_label.setText("✅ Generation completed!")
        else:
...
if __name__ == "__main__":
    ...
    tab.setWindowTitle("🎙️ Chatterbox Voices - Voice Studio")
    tab.resize(1200, 800)
    tab.show()
