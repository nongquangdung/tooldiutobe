...
# -*- coding: utf-8 -*-
"""
[STATS] VOICE STUDIO ANALYTICS TAB - PHASE 4
Professional analytics dashboard với real-time metrics,
production reports, performance analysis, và ROI tracking.
"""
...
class MetricsCard(QFrame):
    ...
    def __init__(self, title: str, value: str, subtitle: str = "", icon: str = "[STATS]"):
        super().__init__()
        self.setup_ui(title, value, subtitle, icon)
...
class AnalyticsTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Title
        title = QLabel("[STATS] Voice Studio Analytics Dashboard")
        title.setStyleSheet("font-size: 24px; font-weight: bold; margin: 16px;")
        layout.addWidget(title)
        ...
        self.files_card = MetricsCard("Files Processed", "0", "Total today", "[FOLDER]")
        self.success_card = MetricsCard("Success Rate", "0%", "Last 24 hours", "[OK]")
        self.quality_card = MetricsCard("Avg Quality", "0.00", "Quality score", "[STAR]")
        self.roi_card = MetricsCard("ROI", "0%", "Return on investment", "[EMOJI]")
        ...
        self.new_session_btn = QPushButton("[REFRESH] Start New Session")
        self.new_session_btn.setStyleSheet("""
            QPushButton {
                background-color: #007bff;
                color: white;
        ...
        self.export_btn = QPushButton("[FILE] Export Report")
        self.export_btn.setStyleSheet("""
            QPushButton {
                background-color: #28a745;
                color: white;
        ...
        # Insights
        insights_title = QLabel("[IDEA] Insights & Recommendations")
        insights_title.setStyleSheet("font-size: 18px; font-weight: bold; margin: 8px;")
        layout.addWidget(insights_title)
        ...
        """)
        self.insights_text.setPlainText("[STATS] Analytics insights will appear here...")
        layout.addWidget(self.insights_text)
        
        # Status
        self.status_label = QLabel("[STATS] Ready - Waiting for analytics data...")
        self.status_label.setStyleSheet("background-color: #e9ecef; padding: 8px; border-radius: 4px;")
        layout.addWidget(self.status_label)
    
    def start_new_session(self):
        """Start new analytics session"""
        self.status_label.setText("[REFRESH] Starting new analytics session...")
        self.files_card.update_value("0", "Session restarted")
        self.success_card.update_value("0%", "No data yet")
        self.quality_card.update_value("0.00", "No data yet")
        self.roi_card.update_value("0%", "No data yet")
        self.insights_text.setPlainText("[REFRESH] New session started...")
    ...
    def export_report(self):
        ...
        if filename:
            try:
                ...
                self.status_label.setText(f"[OK] Report exported: {filename}")
            except Exception as e:
                self.status_label.setText(f"[EMOJI] Export failed: {str(e)}")
    ...
    def update_production_metrics(self, files_processed: int, success_rate: float, 
        ...
        insights = []
        if success_rate > 90:
            insights.append("[OK] Excellent success rate - system performing optimally")
        elif success_rate > 75:
            insights.append("[METRICS] Good success rate - minor optimizations possible")
        else:
            insights.append("[WARNING] Success rate below target - review quality settings")
        
        if avg_quality > 0.9:
            insights.append("[STAR] Outstanding audio quality maintained")
        elif avg_quality > 0.8:
            insights.append("[EMOJI] Good audio quality - meeting standards")
        else:
            insights.append("[TOOL] Audio quality needs improvement")
        
        if roi_percentage > 200:
            insights.append("[EMOJI] Excellent ROI - significant time savings achieved")
