...
class WhisperValidator:
    ...
    def validate_transcription(self, audio_path: str, expected_text: str) -> QualityScore:
        ...
        try:
            ...
            logging.error(f"❌ Transcription validation failed: {e}")
            return QualityScore(
                metric=QualityMetric.TRANSCRIPTION_ACCURACY,
                score=0.0,
                details={"error": str(e)}
...
class QualityController:
    ...
    def __init__(self, 
        ...
        logging.info(f"🎯 Quality Controller initialized - {num_candidates} candidates, {quality_threshold:.1%} threshold")
    ...
    def generate_with_quality_control(self, 
        ...
        logging.info(f"🎯 Starting quality-controlled generation for task: {task_id}")
        ...
        for i in range(self.num_candidates):
            ...
            try:
                ...
                if audio_path and os.path.exists(audio_path):
                    ...
                    logging.info(f"📊 Candidate {i}: Quality {candidate.overall_score:.3f}")
                    
                    # Early exit if high quality found
                    if candidate.is_high_quality:
                        logging.info(f"✅ High quality candidate found early: {candidate.overall_score:.3f}")
                        break
                        
                else:
                    logging.warning(f"⚠️ Failed to generate candidate {i}")
                    
            except Exception as e:
                logging.error(f"❌ Error generating candidate {i}: {e}")
        ...
        if best_candidate and not best_candidate.is_high_quality and len(candidates) < self.max_retries:
            logging.info(f"🔄 Quality below threshold ({best_candidate.overall_score:.3f}), generating additional candidates...")
        ...
        logging.info(f"🎯 Quality control completed: {success_rate:.1%} success, {len(candidates)} candidates, {total_time:.2f}s")
    ...
    def generate_additional_candidates(self, text: str, voice_params: Dict, 
        ...
        for i in range(num_additional):
            try:
                ...
                logging.error(f"❌ Error generating retry candidate {i}: {e}")
...
if __name__ == "__main__":
    ...
    print("🧪 Testing Quality Controller - PHASE 3")
    ...
    print(f"✅ Quality control test completed:")
    print(f"   🎯 Success Rate: {report.success_rate:.1%}")
    print(f"   📊 Best Score: {report.best_candidate.overall_score:.3f if report.best_candidate else 'N/A'}")
    print(f"   🔄 Attempts: {report.total_attempts}")
    print(f"   ⏱️ Time: {report.processing_time:.2f}s")
    ...
    print("\n🎉 Quality Controller PHASE 3 - Implementation Complete!")
